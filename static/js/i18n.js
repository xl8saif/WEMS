...
(function () {
  const U = {
    ...
    'نوٹس': 'Notes', 'کوئی نوٹس نہیں۔': 'No notes.', 'ٹائم لائن': 'Timeline', 'تاریخ': 'Date', 'اجراء': 'Issued', 'اجراء کی تاریخ': 'Issue Date',
    ...
    'آمدن': 'Revenue', 'وصول شدہ': 'Collected', 'ادائیگی کی تاریخ': 'Payment Date', 'طریقۂ ادائیگی': 'Payment Method', 'نقد': 'Cash', 'بینک ٹرانسفر': 'Bank Transfer',
    ...
    'ابھی کوئی خدمت ترتیب نہیں دی گئی۔': 'No services configured yet.', 'دستیاب نہیں': 'Not available',
    ...
    'زیرِ التوا کام': 'Pending Jobs', 'مینو کھولیں': 'Open menu',
  ...
  function translateString(value, map, lang) {
  ...
  function translate(lang) {
    ...
    document.querySelectorAll('[data-lang-label]').forEach(el => { el.textContent = isEnglish ? 'اردو' : 'English'; });
    document.title = translateString(document.title, isEnglish ? U : E, lang);

    document.querySelectorAll('body *:not(script):not(style)').forEach(el => {
      if (el.children.length === 0 && el.textContent.trim()) {
        el.textContent = translateString(el.textContent, isEnglish ? U : E, lang);
      }
      ['placeholder', 'title', 'aria-label'].forEach(attr => {
        if (el.hasAttribute(attr)) el.setAttribute(attr, translateString(el.getAttribute(attr), isEnglish ? U : E, lang));
      });
    });

    document.querySelectorAll('[onsubmit]').forEach(el => {
      el.setAttribute('onsubmit', translateString(el.getAttribute('onsubmit'), isEnglish ? U : E, lang));
    });
